<?php
	...
	function replaceTimLoga($log_desc){
			...
			$log_desc = str_replace("tima 4","tima <b>SSC</b>",$log_desc);
			$log_desc = str_replace("tima 5","tima <b>JSI</b>",$log_desc);
		return $log_desc;
	}
	...
	<div id="content">
									...
									<div class="col-md-8">
										<div class="row">
											...
											<div class="text-center">
												<a href="<?php getSiteURL(); ?>dipl_agenti?page=arhivirani_agenti" style="float: right; margin-right: 8px;" class="btn material-btn material-btn-icon-success material-btn_success main-container__column material-btn-icon-responsive"><i class="fa fa-tty" aria-hidden="true"></i> <span>Arhivirani agenti</span></a>
											</div>
											<?php
											}
											if(in_array( "1" , $employee_status) OR $logged_employee_id == '201'){
											?>
											<div class="text-center">
								...
								<script type="text/javascript">
									$(document).ready(function() {
										...
										$('.editLimitsAll').on('click', function(){
											...
											$("#check_campaign_a").prop('checked', false);
											$("#check_campaign_b").prop('checked', false);
											$("#check_campaign_c").prop('checked', false);
											$("#display_dnevni").hide();
										...
										$('.uredi_samo_limite').on('click', function(){
											...
											var edit_category = $('input[name=edit_category_Q').val();
											var edit_limit = $('input[name=edit_limit_Q').val();
											var edit_drzave = $('input[name=edit_drzave_Q').val();
											if(edit_category || edit_limit || edit_drzave){
												$(".submit_btn").prop('disabled', false);
												$(".submit_btn").removeClass('material-btn_primary');
												$(".submit_btn").addClass('material-btn_info');
											}
										...
										$('.edit_category ').on('click', function(){
											...
											$("#campaign_category").toggle();
											$("#drugi_tekst_c").toggle();
											$("#prvi_tekst_c").toggle();
											var edit_category = $('input[name=edit_category_Q').val();
											$('input[name=edit_category_Q').val(edit_category === "0" ? "1" : "0");
											var edit_limit = $('input[name=edit_limit_Q').val();
											var edit_drzave = $('input[name=edit_drzave_Q').val();
											if(edit_category || edit_limit || edit_drzave){
												$(".submit_btn").prop('disabled', false);
								...
								<table id="idk_table" class="display" cellspacing="0" width="100%">
									<thead>
										<tr>
											...
											<th class="text-center">Leadovi + NK1 + NL1</th>
											<th></th>
										</tr>
									</thead>
									<tbody>
										...
										$query = $db->prepare("
															SELECT e.employee_id, e.employee_firstname, e.employee_lastname, e.employee_status, e.employee_image, e.employee_nostrifikacija_diploma,
																	lt_drzava, lt_dnevni, lt_sedmicni, lt_mjesecni, lt_dnevni_cnt, lt_sedmicni_cnt, lt_mjesecni_cnt, lt_campaign_category
															FROM idk_nd_limiti 
										...
										while($row = $query->fetch()){
											...
											$lt_campaign_category = $row['lt_campaign_category'];
											$lt_campaign_category_string = explode(',', $lt_campaign_category);
											$lt_drzava_img = "";
											if(in_array( "bih" , $lt_drzava)){
												$lt_drzava_img .= ' <img src="'.getSiteUrlr().'images/bs3d.png" width=25>';
											}
											if(in_array( "srb" , $lt_drzava)){
												$lt_drzava_img .= ' <img src="'.getSiteUrlr().'images/sr3d.png" width=25>';
											}
											if(in_array( "de" , $lt_drzava)){
												$lt_drzava_img .= ' <img src="'.getSiteUrlr().'images/de3d.png" width=25>';
											}
											if(in_array( "ostalo" , $lt_drzava)){
												$lt_drzava_img .= ' <img src="'.getSiteUrlr().'images/globe3d.png" title="Ostale zemlje" width=25>';
											}
											
											$campaign_category_cell = "";
											if(in_array( "A" , $lt_campaign_category_string)){
												$campaign_category_cell .= '<a class = "campaign-category campaign-A">A</a>';
											}
											if(in_array( "B" , $lt_campaign_category_string)){
												$campaign_category_cell .= '<a class = "campaign-category campaign-B">B</a>';
											}
											if(in_array( "C" , $lt_campaign_category_string)){
												$campaign_category_cell .= '<a class = "campaign-category campaign-C">C</a>';
											}
											$query_leads = $db->prepare("
														SELECT id_broj_nd_kandidata
														FROM idk_nd_kandidata
														WHERE zaduzen_zaposlenik_nd_kandidata = :zaduzen_zaposlenik_nd_kandidata AND status_nd_kandidata = 1 AND (pstatus_nd_kandidata = 1 OR pstatus_nd_kandidata = 6 OR pstatus_nd_kandidata = 7)
											");
										...
										?>
											<tr>
												...
												<td><?php echo $campaign_category_cell; ?></td>
												<td class="text-center">
													<div class="progress" style="position:relative; margin-bottom: 0;">
														...
														<div class="progress-bar progress-bar-<?php echo $stil_dnevni?>" role="progressbar" aria-valuenow="<?php echo ($lt_dnevni == 0) ? '0' : (($lt_dnevni_cnt/$lt_dnevni)*100); ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo ($lt_dnevni == 0) ? '0' : (($lt_dnevni_cnt/$lt_dnevni)*100); ?>%;">
														</div>
													</div>
												</td>
												<td class="text-center">
													<div class="progress" style="position:relative; margin-bottom: 0;">
														...
														<div class="progress-bar progress-bar-<?php echo $stil_sedmicni?>" role="progressbar" aria-valuenow="<?php echo ($lt_sedmicni == 0) ? '0' :($lt_sedmicni_cnt/$lt_sedmicni)*100; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo ($lt_sedmicni == 0) ? '0' :($lt_sedmicni_cnt/$lt_sedmicni)*100; ?>%;">
														</div>
													</div>
												</td>
												<td class="text-center">
													<div class="progress" style="position:relative; margin-bottom: 0;">
														...
														<div class="progress-bar progress-bar-<?php echo $stil_mjesecni?>" role="progressbar" aria-valuenow="<?php echo ($lt_mjesecni == 0) ? '0': ($lt_mjesecni_cnt/$lt_mjesecni)*100; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo ($lt_mjesecni == 0) ? '0': ($lt_mjesecni_cnt/$lt_mjesecni)*100 ?>%;">
														</div>
													</div>
												</td>
												<td class="text-center">
													...
													<div class="btn-group material-btn-group">
														...
														<ul class="dropdown-menu material-dropdown-menu material-dropdown-menu_primary idk_dropdown_table" role="menu">
															...
															<li><a href="#" class="material-dropdown-menu__link editLimits"
																	...
																	data-campaign_category="<?php echo $lt_campaign_category; ?>" 
																	>
										...
										<div class="modal-content material-modal__content">
											...
											<form action="<?php getSiteURL(); ?>dipl_agenti?page=editLimits" method="post" http-equiv="Content-type" enctype="multipart/form-data" charset="UTF-8" class="form-horizontal" id="form_edit_limits">
												<div class="modal-body material-modal__body">	
														...
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="bih_check" class="col-sm-6 control-label">
																	BiH   <img src="<?php getSiteURL(); ?>images/bs3d.png" width=25>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
														...
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="srb_check" class="col-sm-6 control-label">
																	Srbija   <img src="<?php getSiteURL(); ?>images/sr3d.png" width=25>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
														...
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="de_check" class="col-sm-6 control-label">
																	Njemačka   <img src="<?php getSiteURL(); ?>images/de3d.png" width=25>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
														...
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="ostalo_check" class="col-sm-6 control-label">
																	Ostale   <img src="<?php getSiteURL(); ?>images/globe3d.png" width=25>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
													...
													<hr>
													<input type="hidden" id="edit_category_Q" name="edit_category_Q">
													<div id="edit_category" class="text-center " style="margin-bottom: 10px;">
														<button type="button" class="edit_category btn material-btn material-btn_light material-btn-icon-primary material-btn-icon-responsive"><span id="prvi_tekst_c"><i class="fa fa-chevron-down" aria-hidden="true"></i>Uredi kategorije</span><span id="drugi_tekst_c"><i class="fa fa-chevron-up" aria-hidden="true"></i>Sakrij kategorije (neće se ažurirati)</span></button>
													</div>
													<div id="campaign_category">
														<div class="col-md-offset-1 col-sm-10 text-center">
															<div class="col-sm-12">
																<h4>Kategorije koje agent zove:</h4>
															</div>
														</div>
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="category_a" class="col-sm-6 control-label">
																	<a class = "campaign-category campaign-A">A</a>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
																		<input class="materail-switch__element" type="checkbox" id="check_campaign_a" name="check_campaign_a" >
																		<label class="materail-switch__label" for="check_campaign_a"></label>
																	</div>
																</div>
															</div>
														</div>
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="category_b" class="col-sm-6 control-label">
																	<a class = "campaign-category campaign-B">B</a>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
																		<input class="materail-switch__element" type="checkbox" id="check_campaign_b" name="check_campaign_b" >
																		<label class="materail-switch__label" for="check_campaign_b"></label>
																	</div>
																</div>
															</div>
														</div>
														<div class="form-group" style="margin-bottom: 10px;">
															<div class="col-md-offset-1 col-sm-10">
																<label for="category_c" class="col-sm-6 control-label">
																	<a class = "campaign-category campaign-C">C</a>
																</label>
																<div class="col-sm-6 ">
																	<div class="main-container__column materail-switch materail-switch_primary">
																		<input class="materail-switch__element" type="checkbox" id="check_campaign_c" name="check_campaign_c" >
																		<label class="materail-switch__label" for="check_campaign_c"></label>
																	</div>
																</div>
															</div>
														</div>
											...
											<script>
												$(document).on("click",".editLimits",function() {
													...
													$('input[name=edit_category_Q').val("1");
													...
													var campaign_category = $(this).data("campaign_category");
													var drzave_array = drzave_string.split(",");
													var campaign_category = campaign_category.split(",");
													...
													console.log(jQuery.inArray("A", campaign_category));
													if(jQuery.inArray("A", campaign_category) !== -1){
														$("#check_campaign_a").prop('checked', true);
													}
													if(jQuery.inArray("B", campaign_category) !== -1){
														$("#check_campaign_b").prop('checked', true);
													}
													if(jQuery.inArray("C", campaign_category) !== -1){
														$("#check_campaign_c").prop('checked', true);
													}
		...
		<?php
				...
				case "open_skladiste":
					...
					//if((in_array( "9" , $employee_status)) OR (in_array( "1" , $employee_status)) OR (!empty($employee_supervizor[0]))){
					if((in_array( "1" , $employee_status)) OR $logged_employee_id == 67)
						$uslov_team = "";
					else
												...
												<div class="panel-heading material-accordion__heading">
													<h4 class="panel-title">
														...
														<a class="material-accordion__title " style="padding: 15px 30px 35px 15px;" data-toggle="collapse" data-parent="#accordion0" href="#<?php echo "tim0"; ?>">
															...
															<span class="col-xs-1"><?php echo "UK: ". getBrojKandidata(11, '000', '0'); ?></span>
															<span class="col-xs-1"><?php echo "BH: ". getBrojKandidata(11, '+387', '0'); ?></span>
															<span class="col-xs-1"><?php echo "SR: ".getBrojKandidata(11, '+381', '0'); ?></span>
															<span class="col-xs-1"><?php echo "DE: ".getBrojKandidata(11, '+49', '0'); ?></span>
															<span class="col-xs-1"><?php echo "OS: ".getBrojKandidata(11, '111', '0'); ?></span>
														</a>
														</div>
													</h4>
												</div>
												...
												<div id="<?php echo "tim0"; ?>" class="panel-collapse collapse material-accordion__collapse">
													<div class="panel-body">
															...
															<table class="table table-striped">
																...
																<tbody>
																	<tr>
																		...
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(11, '000', '0'); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(11, '+387', '0'); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(11, '+381', '0'); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(11, '+49', '0'); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(11, '111', '0'); ?></td>
																	</tr>
																</tbody>
															</table>
														</div>
														<div class="col-sm-10 col-md-10 col-lg-5" style="border: 1px solid #cccccc; margin: 15px 5px; padding: 20px; border-radius: 1.25rem; box-shadow: 0 0 10px #cccccc;">
															...
															<form action="<?php getSiteURL(); ?>do_dipl?form=prebaci_iz_skladista&team=0" method="post" http-equiv="Content-type" enctype="multipart/form-data" charset="UTF-8" class="form-horizontal" id="form_prebaci_iz_skladista<?php echo $id_t; ?>">
																<div class="form-group">
																	<div class="col-md-offset-1 col-sm-10">
													...
													<div class="panel-body">
														<div class="col-sm-10 col-md-10 col-lg-6 table-responsive" style="border: 1px solid #cccccc; margin: 15px 5px; padding: 20px; border-radius: 1.25rem; box-shadow: 0 0 10px #cccccc;">
															<table class="table table-striped">
																...
																<tbody>
																	...
																	<tr>
																		<td class="text-center"><span style = "background-color: #00fafb; color: white" class="label label-default material-label material-label_default main-container__column text-left">Neuspješan Kontakt 1</span></td>
																	...
																	<tr>
																		<td class="text-center"><span style = "background-color: #00fb53; color: white" class="label label-default material-label material-label_default main-container__column text-left">Neuspješan Kontakt 3</span></td>
																	...
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,7);?></td>
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(17, '000', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(17, '+387', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(17, '+381', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(17, '+49', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(17, '111', $id_t); ?></td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,8);?></td>
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(18, '000', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(18, '+387', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(18, '+381', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(18, '+49', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(18, '111', $id_t); ?></td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,9);?></td>
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(19, '000', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(19, '+387', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(19, '+381', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(19, '+49', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(19, '111', $id_t); ?></td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,10);?></td>
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(110, '000', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(110, '+387', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(110, '+381', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(110, '+49', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(110, '111', $id_t); ?></td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,11);?></td>
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(111, '000', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(111, '+387', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(111, '+381', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(111, '+49', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(111, '111', $id_t); ?></td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,12);?></td>
																		<td class="text-center" style = "font-weight: bold;"><?php echo getBrojKandidata(112, '000', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(112, '+387', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(112, '+381', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(112, '+49', $id_t); ?></td>
																		<td class="text-center"><?php echo getBrojKandidata(112, '111', $id_t); ?></td>
																	</tr>
														...
														<div class="col-sm-10 col-md-10 col-lg-5" style="border: 1px solid #cccccc; margin: 15px 5px; padding: 20px; border-radius: 1.25rem; box-shadow: 0 0 10px #cccccc;">
															...
															<form action="<?php getSiteURL(); ?>do_dipl?form=prebaci_iz_skladista&team=<?php echo $id_t; ?>" method="post" http-equiv="Content-type" enctype="multipart/form-data" charset="UTF-8" class="form-horizontal" id="form_prebaci_iz_skladista<?php echo $id_t; ?>">
																...
																<div class="form-group style_razlozi_nezainteresiran_lead<?php echo $id_t; ?>" >
																	<div class="col-md-offset-1 col-sm-10">
																		<label for="select_razlog_nezainteresiran_lead<?php echo $id_t;?>" class="col-sm-4 control-label" style="padding-left: 0px;">
																			Nezainteresiran lead (razlozi):
																		</label>
																		<div class="col-sm-6" style="margin-bottom: 15px;">
																			<select class="selectpicker" id="select_razlog_nezainteresiran_lead<?php echo $id_t;?>" name="select_razlog_nezainteresiran_lead<?php echo $id_t;?>">
																			</select>
																		</div>
																	</div>
																</div>
																<div class="form-group style_razlozi_arhiva<?php echo $id_t; ?>" >
																	<div class="col-md-offset-1 col-sm-10">
																		<label for="select_razlog_arhiva<?php echo $id_t;?>" class="col-sm-4 control-label" style="padding-left: 0px;">
																			Arhivirani (razlozi):
																		</label>
																		<div class="col-sm-6" style="margin-bottom: 15px;">
																			<select class="selectpicker" id="select_razlog_arhiva<?php echo $id_t;?>" name="select_razlog_arhiva<?php echo $id_t;?>">
																			</select>
																		</div>
																	</div>
																</div>
																...
																<script>
																	$(document).ready(function() {
																		...
																		$('.style_razlozi_arhiva<?php echo $id_t; ?>').hide();
																		$('.style_razlozi_nezainteresiran_lead<?php echo $id_t; ?>').hide();
																	...
																	$("#select_statusi<?php echo $id_t; ?>").change(function() {
																		...
																		//console.log(drzava_val);
																		if(status_val != null && status_val.includes("14")){
																			const index = status_val.indexOf("14");
																			if (index > -1) {
																			  status_val.splice(index, 1);
																			}
																			$.ajax({
																				url: 'ajax_data.php?page=getRazloziNezainteresiranLead',
																				type: 'POST',
																				data: {'tim_id': <?php echo $id_t?>, 'drzava_val': drzava_val},
																				dataType: 'html',
																				success: function(data) {
																					$(".style_razlozi_nezainteresiran_lead<?php echo $id_t; ?>").show();
																					$("#select_razlog_nezainteresiran_lead<?php echo $id_t; ?>").html(data).selectpicker('refresh');
																		...
																		if(status_val != null && status_val.includes("7")){
																			const index = status_val.indexOf("7");
																			if (index > -1) {
																			  status_val.splice(index, 1);
																			}
																			$.ajax({
																				url: 'ajax_data.php?page=getRazloziArhiva',
																				type: 'POST',
																				data: {'tim_id': <?php echo $id_t?>, 'drzava_val': drzava_val},
																				dataType: 'html',
																				success: function(data) {
																					$(".style_razlozi_arhiva<?php echo $id_t; ?>").show();
																					$("#select_razlog_arhiva<?php echo $id_t; ?>").html(data).selectpicker('refresh');
																	...
																	$('#select_razlog_nezainteresiran_lead<?php echo $id_t; ?>, #select_razlog_arhiva<?php echo $id_t; ?>').on('change', function(){
																		var status_val = $('#select_statusi<?php echo $id_t; ?>').val();
																		var arhiva_razlozi_id = $('#select_razlog_arhiva<?php echo $id_t?>').val();
																		var nezainteresirani_lead_razlozi_id = $('#select_razlog_nezainteresiran_lead<?php echo $id_t?>').val();
																		var drzava_val = $(document).find('#select_drzava<?php echo $id_t?>').val();
																		$.ajax({
																			url: 'ajax_data.php?page=getStatusiBox',
																			type: 'POST',
																			data: {'status_val':status_val, 
																				'tim_id': <?php echo $id_t?>, 
																				'drzava_val': drzava_val,
																				'arhiva_razlozi_id': arhiva_razlozi_id,
				...
				case "editLimits":
					...
					$edit_category_Q = $_POST['edit_category_Q'];
					...
					$check_campaign_a = $_POST['check_campaign_a'];
					$check_campaign_b = $_POST['check_campaign_b'];
					$check_campaign_c = $_POST['check_campaign_c'];
					
					$campaign_checked = array();
					if($check_campaign_a == "on")
						array_push($campaign_checked, "A");
					if($check_campaign_b == "on")
						array_push($campaign_checked, "B");
					if($check_campaign_c == "on")
						array_push($campaign_checked, "C");
					...
					$q_campaign = "";
					if($edit_category_Q != "0")
						$q_campaign = "lt_campaign_category = '".implode(',', $campaign_checked)."'";
					
					if($q_dr != "" AND ($q_lt != "" OR $q_campaign != ""))
						$q_dr = $q_dr.",";
					
					if($q_lt != "" AND $q_campaign != "")
						$q_lt = $q_lt.",";
					
					$query_set=$db->prepare("
						UPDATE idk_nd_limiti
						SET ".$q_dr.$q_lt.$q_campaign."
						WHERE ".$q_id
					
					);
					
					// var_dump($query_set);
					// var_dump($edit_limit_Q);
					// var_dump($edit_category_Q);
					// var_dump($edit_drzave_Q);
					// exit();
											...
											<div class = "col-xs-12">
												...
												<div class="row" style = "padding-top: 10px;">
													<div class = "col-xs-12">
														...
														sum(case when status_nd_kandidata=1 AND pstatus_nd_kandidata=7 AND zaduzen_zaposlenik_nd_kandidata = '.$id.' THEN 1 else 0 END) AS Neuld1,
														sum(case when status_nd_kandidata=1 AND pstatus_nd_kandidata=8 AND zaduzen_zaposlenik_nd_kandidata = '.$id.' THEN 1 else 0 END) AS Neuld2,
														sum(case when status_nd_kandidata=1 AND pstatus_nd_kandidata=9 AND zaduzen_zaposlenik_nd_kandidata = '.$id.' THEN 1 else 0 END) AS Terzai,
														sum(case when status_nd_kandidata=1 AND pstatus_nd_kandidata=10 AND zaduzen_zaposlenik_nd_kandidata = '.$id.' THEN 1 else 0 END) AS Terost,
														sum(case when status_nd_kandidata=1 AND pstatus_nd_kandidata=11 AND zaduzen_zaposlenik_nd_kandidata = '.$id.' THEN 1 else 0 END) AS LeadNL,
														sum(case when status_nd_kandidata=1 AND pstatus_nd_kandidata=12 AND zaduzen_zaposlenik_nd_kandidata = '.$id.' THEN 1 else 0 END) AS LeadNZ,
														...
														$Neuld1 = $row_cntst['Neuld1'];
														$Neuld2 = $row_cntst['Neuld2'];
														$Terzai = $row_cntst['Terzai'];
														$Terost = $row_cntst['Terost'];
														$LeadNL = $row_cntst['LeadNL'];
														$LeadNZ = $row_cntst['LeadNZ'];
                                                        ...
                                                        <script>
															...
															<table class="table table-striped" style = "margin-bottom: 0px;">
																...
																<tbody>
																	...
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,7);?></td>
																		<td class="text-center max_st" style = "font-weight: bold;" id="td_Neuld1"><?php echo $Neuld1;?></td>
																		<td>
																			<div class="main-container__column materail-switch materail-switch_primary">
																				<input class="materail-switch__element" type="checkbox" id="check_Neuld1" name="check_Neuld1" >
																				<label class="materail-switch__label" for="check_Neuld1"></label>
																			</div>
																		</td>
																		<td>
																			<div id="uredi_Neuld1" style="display: none;">
																				<div class="col-sm-2 text-right">
																					<a id = "minus_Neuld1" class="plus_minus_prebaci btn material-btn btn-danger" ><i class="fa fa-minus" aria-hidden="true"></i></a>
																				</div>
																				<div class="col-sm-5 col-sm-offset-1 text-center">
																					<div class="materail-input-block materail-input-block_success">
																						<input value="0" name="input_Neuld1" id="input_Neuld1" style = "text-align: center; font-weight: bold;border-radius:15px; border-bottom: 2px solid #4092d9;" class="form-control materail-input br_kand" type="number" min = "0" required>
																					</div>
																				</div>
																				<div class="col-sm-2 text-left">
																					<a id = "plus_Neuld1" class="plus_minus_prebaci btn material-btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a> 
																				</div>
																			</div>
																		</td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,8);?></td>
																		<td class="text-center max_st" style = "font-weight: bold;" id="td_Neuld2"><?php echo $Neuld2;?></td>
																		<td>
																			<div class="main-container__column materail-switch materail-switch_primary">
																				<input class="materail-switch__element" type="checkbox" id="check_Neuld2" name="check_Neuld2" >
																				<label class="materail-switch__label" for="check_Neuld2"></label>
																			</div>
																		</td>
																		<td>
																			<div id="uredi_Neuld2" style="display: none;">
																				<div class="col-sm-2 text-right">
																					<a id = "minus_Neuld2" class="plus_minus_prebaci btn material-btn btn-danger" ><i class="fa fa-minus" aria-hidden="true"></i></a>
																				</div>
																				<div class="col-sm-5 col-sm-offset-1 text-center">
																					<div class="materail-input-block materail-input-block_success">
																						<input value="0" name="input_Neuld2" id="input_Neuld2" style = "text-align: center; font-weight: bold;border-radius:15px; border-bottom: 2px solid #4092d9;" class="form-control materail-input br_kand" type="number" min = "0" required>
																					</div>
																				</div>
																				<div class="col-sm-2 text-left">
																					<a id = "plus_Neuld2" class="plus_minus_prebaci btn material-btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a> 
																				</div>
																			</div>
																		</td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,9);?></td>
																		<td class="text-center max_st" style = "font-weight: bold;" id="td_Terzai"><?php echo $Terzai;?></td>
																		<td>
																			<div class="main-container__column materail-switch materail-switch_primary">
																				<input class="materail-switch__element" type="checkbox" id="check_Terzai" name="check_Terzai" >
																				<label class="materail-switch__label" for="check_Terzai"></label>
																			</div>
																		</td>
																		<td>
																			<div id="uredi_Terzai" style="display: none;">
																				<div class="col-sm-2 text-right">
																					<a id = "minus_Terzai" class="plus_minus_prebaci btn material-btn btn-danger" ><i class="fa fa-minus" aria-hidden="true"></i></a>
																				</div>
																				<div class="col-sm-5 col-sm-offset-1 text-center">
																					<div class="materail-input-block materail-input-block_success">
																						<input value="0" name="input_Terzai" id="input_Terzai" style = "text-align: center; font-weight: bold;border-radius:15px; border-bottom: 2px solid #4092d9;" class="form-control materail-input br_kand" type="number" min = "0" required>
																					</div>
																				</div>
																				<div class="col-sm-2 text-left">
																					<a id = "plus_Terzai" class="plus_minus_prebaci btn material-btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a> 
																				</div>
																			</div>
																		</td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,10);?></td>
																		<td class="text-center max_st" style = "font-weight: bold;" id="td_Terost"><?php echo $Terost;?></td>
																		<td>
																			<div class="main-container__column materail-switch materail-switch_primary">
																				<input class="materail-switch__element" type="checkbox" id="check_Terost" name="check_Terost" >
																				<label class="materail-switch__label" for="check_Terost"></label>
																			</div>
																		</td>
																		<td>
																			<div id="uredi_Terost" style="display: none;">
																				<div class="col-sm-2 text-right">
																					<a id = "minus_Terost" class="plus_minus_prebaci btn material-btn btn-danger" ><i class="fa fa-minus" aria-hidden="true"></i></a>
																				</div>
																				<div class="col-sm-5 col-sm-offset-1 text-center">
																					<div class="materail-input-block materail-input-block_success">
																						<input value="0" name="input_Terost" id="input_Terost" style = "text-align: center; font-weight: bold;border-radius:15px; border-bottom: 2px solid #4092d9;" class="form-control materail-input br_kand" type="number" min = "0" required>
																					</div>
																				</div>
																				<div class="col-sm-2 text-left">
																					<a id = "plus_Terost" class="plus_minus_prebaci btn material-btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a> 
																				</div>
																			</div>
																		</td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,11);?></td>
																		<td class="text-center max_st" style = "font-weight: bold;" id="td_LeadNL"><?php echo $LeadNL;?></td>
																		<td>
																			<div class="main-container__column materail-switch materail-switch_primary">
																				<input class="materail-switch__element" type="checkbox" id="check_LeadNL" name="check_LeadNL" >
																				<label class="materail-switch__label" for="check_LeadNL"></label>
																			</div>
																		</td>
																		<td>
																			<div id="uredi_LeadNL" style="display: none;">
																				<div class="col-sm-2 text-right">
																					<a id = "minus_LeadNL" class="plus_minus_prebaci btn material-btn btn-danger" ><i class="fa fa-minus" aria-hidden="true"></i></a>
																				</div>
																				<div class="col-sm-5 col-sm-offset-1 text-center">
																					<div class="materail-input-block materail-input-block_success">
																						<input value="0" name="input_LeadNL" id="input_LeadNL" style = "text-align: center; font-weight: bold;border-radius:15px; border-bottom: 2px solid #4092d9;" class="form-control materail-input br_kand" type="number" min = "0" required>
																					</div>
																				</div>
																				<div class="col-sm-2 text-left">
																					<a id = "plus_LeadNL" class="plus_minus_prebaci btn material-btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a> 
																				</div>
																			</div>
																		</td>
																	</tr>
																	<tr>
																		<td class="text-center"><?php echo getStatusDIPLKandidatR(1,12);?></td>
																		<td class="text-center max_st" style = "font-weight: bold;" id="td_LeadNZ"><?php echo $LeadNZ;?></td>
																		<td>
																			<div class="main-container__column materail-switch materail-switch_primary">
																				<input class="materail-switch__element" type="checkbox" id="check_LeadNZ" name="check_LeadNZ" >
																				<label class="materail-switch__label" for="check_LeadNZ"></label>
																			</div>
																		</td>
																		<td>
																			<div id="uredi_LeadNZ" style="display: none;">
																				<div class="col-sm-2 text-right">
																					<a id = "minus_LeadNZ" class="plus_minus_prebaci btn material-btn btn-danger" ><i class="fa fa-minus" aria-hidden="true"></i></a>
																				</div>
																				<div class="col-sm-5 col-sm-offset-1 text-center">
																					<div class="materail-input-block materail-input-block_success">
																						<input value="0" name="input_LeadNZ" id="input_LeadNZ" style = "text-align: center; font-weight: bold;border-radius:15px; border-bottom: 2px solid #4092d9;" class="form-control materail-input br_kand" type="number" min = "0" required>
																					</div>
																				</div>
																				<div class="col-sm-2 text-left">
																					<a id = "plus_LeadNZ" class="plus_minus_prebaci btn material-btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a> 
																				</div>
																			</div>
																		</td>
																	</tr>
															...
															<script>
																...
																function count_checks(){
																	...
																	if(status==0){
																		var checks=[
																			...
																			document.getElementById("check_Neuld1").checked,
																			document.getElementById("check_Neuld2").checked,
																			document.getElementById("check_Terzai").checked,
																			document.getElementById("check_LeadNL").checked,
																			document.getElementById("check_LeadNZ").checked,
																			document.getElementById("check_Terost").checked
																		]
																...
																$(document).on("click","#plus_Neuld1",function(){
																	inc("input_Neuld1","td_Neuld1");
																});
																$(document).on("click","#plus_Neuld2",function(){
																	inc("input_Neuld2","td_Neuld2");
																});
																$(document).on("click","#plus_Terzai",function(){
																	inc("input_Terzai","td_Terzai");
																});
																$(document).on("click","#plus_Terost",function(){
																	inc("input_Terost","td_Terost");
																});
																$(document).on("click","#plus_LeadNL",function(){
																	inc("input_LeadNL","td_LeadNL");
																});
																$(document).on("click","#plus_LeadNZ",function(){
																	inc("input_LeadNZ","td_LeadNZ");
																});
																...
																$(document).on("click","#minus_Neuld1",function(){
																	dec("input_Neuld1");
																});
																$(document).on("click","#minus_Neuld2",function(){
																	dec("input_Neuld2");
																});
																$(document).on("click","#minus_Terzai",function(){
																	dec("input_Terzai");
																});
																$(document).on("click","#minus_Terost",function(){
																	dec("input_Terost");
																});
																$(document).on("click","#minus_LeadNL",function(){
																	dec("input_LeadNL");
																});
																$(document).on("click","#minus_LeadNZ",function(){
																	dec("input_LeadNZ");
																});
																...
																$(document).on("click","#check_Neuld1",function() {
																	count_checks();
																	var x = document.getElementById("uredi_Neuld1");
																...
																$(document).on("click","#check_Neuld2",function() {
																	count_checks();
																	var x = document.getElementById("uredi_Neuld2");
																	var y = document.getElementById("btn_prebaci");
																	if(x.style.display=="none"){
																		x.style.display="block";
																		y.style.display="block";
																...
																$(document).on("click","#check_Terzai",function() {
																	count_checks();
																	var x = document.getElementById("uredi_Terzai");
																	var y = document.getElementById("btn_prebaci");
																	if(x.style.display=="none"){
																		x.style.display="block";
																		y.style.display="block";
																...
																$(document).on("click","#check_Terost",function() {
																	count_checks();
																	var x = document.getElementById("uredi_Terost");
																	var y = document.getElementById("btn_prebaci");
																	if(x.style.display=="none"){
																		x.style.display="block";
																		y.style.display="block";
																...
																$(document).on("click","#check_LeadNL",function() {
																	count_checks();
																	var x = document.getElementById("uredi_LeadNL");
																	var y = document.getElementById("btn_prebaci");
																	if(x.style.display=="none"){
																		x.style.display="block";
																		y.style.display="block";
																...
																$(document).on("click","#check_LeadNZ",function() {
																	count_checks();
																	var x = document.getElementById("uredi_LeadNZ");
																	var y = document.getElementById("btn_prebaci");
																	if(x.style.display=="none"){
																		x.style.display="block";
																		y.style.display="block";
																...
																$(document).on("click","#btn_prebaci",function(){
																	...
																	if(document.getElementById("check_Neuld1").checked){
																		qhelper[i]="Neuld1,"+document.getElementById("input_Neuld1").value;
																		i=i+1;
																	}
																	if(document.getElementById("check_Neuld2").checked){
																		qhelper[i]="Neuld2,"+document.getElementById("input_Neuld2").value;
																		i=i+1;
																	}
																	if(document.getElementById("check_Terzai").checked){
																		qhelper[i]="Terzai,"+document.getElementById("input_Terzai").value;
																		i=i+1;
																	}
																	if(document.getElementById("check_Terost").checked){
																		qhelper[i]="Terost,"+document.getElementById("input_Terost").value;
																		i=i+1;
																	}
																	if(document.getElementById("check_LeadNL").checked){
																		qhelper[i]="LeadNL,"+document.getElementById("input_LeadNL").value;
																		i=i+1;
																	}
																	if(document.getElementById("check_LeadNZ").checked){
																		qhelper[i]="LeadNZ,"+document.getElementById("input_LeadNZ").value;
																		i=i+1;
																	}
											...
											<table id="skl_u_skl" class="display" cellspacing="0" width="100%">
												...
												<tbody>
													...
													$query_skl_skl = $db->prepare("
														SELECT log_employeeid, log_desc, date(log_date) as log_date FROM `idk_logs` WHERE `log_desc` LIKE '%skladište tima%'  AND log_date between date(current_date() - interval 14 day) AND date(current_date() + interval + 1 day) ORDER BY `log_date` ASC
													");
											...
											<table id="skl_u_zap" class="display" cellspacing="0" width="100%">
												...
												<tbody>
													...
													$query_skl_zap = $db->prepare("
														SELECT log_employeeid, log_desc, date(log_date) as log_date FROM `idk_logs` WHERE `log_desc` LIKE '%iz skladišta zaposleniku%' AND log_date between date(current_date() - interval 14 day) AND date(current_date() + interval + 1 day) ORDER BY `log_date` ASC
													");
											...
											<table id="zap_u_zap" class="display" cellspacing="0" width="100%">
												...
												<tbody>
													...
													$query_zap_zap = $db->prepare("
														SELECT log_employeeid, log_desc, date(log_date) as log_date FROM `idk_logs` WHERE `log_desc` LIKE '%prebacio zaduženje nostrifikacije diplome%'  AND log_date between date(current_date() - interval 14 day) AND date(current_date() + interval + 1 day)ORDER BY `log_date` ASC
													");
											...
											<table id="zap_u_skl" class="display" cellspacing="0" width="100%">
												...
												<tbody>
													...
													$query_zap_skl = $db->prepare("
														SELECT log_employeeid, log_desc, date(log_date) as log_date FROM idk_logs WHERE log_desc LIKE '%prebacuje kandidate%' AND log_desc LIKE '%sa agenta%'  AND log_date between date(current_date() - interval 14 day) AND date(current_date() + interval + 1 day)
													...
													while($row_zap_skl = $query_zap_skl->fetch()){
														...
														$log_desc = str_replace(getStringBetween($log_desc, getZaposlenikimeR($row_zap_skl['log_employeeid']), "reb"), " p", $log_desc);
...
//TABELA LOGOVI PREBACIVANJA KANDIDATA SA NEKOG ZAPOSLENIKA U SKLADIŠTE_______________________________________________________________________________________END			
				break;
				case "arhivirani_agenti":

				?>
				
				<div class="row">
					<div class="col-xs-12 text-left">
						<h1><i class="fa fa-tty idk_color_green" aria-hidden="true"></i> Arhivirani agenti </h1>
						<hr>
					</div>
				</div>
				<div id="arhivirani_zaposlenici_id" style="display: none;"></div>
				<script>
				$(document).ready(function() { 
					var arhivirani_tabela = $('#arhivirani_tabela').DataTable({
						...
						"aoColumns": [
								...
								{ "width": "40%" },
								{ "width": "11%"},
								{ "width": "18%", "bSortable": false}

							]
					});
							
					$('.prebacivanje_oznacenih').on('click', function(){
		
						$("#arhivirani_zaposlenici_id.checkbox").remove();
						
						var rows_selected = arhivirani_tabela.column(0).checkboxes.selected();		
						// Iterate over all selected checkboxes
						$.each(rows_selected, function(index, rowId){
							$("#arhivirani_zaposlenici_id").append(
								rowId
							);
						});
						
						var ids = $("#arhivirani_zaposlenici_id").find(".checkbox").map(function(){return $(this).val(); }).get();
						$.ajax({
							url: 'ajax_data.php?page=prebaci_sve_skladiste',
							type: 'POST',
							data: {'agent_ids':ids},
							dataType: 'html',
							success: function(data) {
								alert("Uspješno prebačeni svi kandidati na skladište");
								location.reload();
							},
							error: function (xhr, ajaxOptions, thrownError) {
								alert(xhr.status);
							...
							<div class="col-xs-12">
								<div class="row">
									...
									<div class="col-md-8">
										<div class="row">
											<div class="text-center">
												<a href="<?php getSiteURL(); ?>dipl_agenti?page=list" style="float: right; margin-right: 8px;" class="btn material-btn material-btn-icon-primary material-btn_primary main-container__column material-btn-icon-responsive"><i class="fa fa-arrow-left" aria-hidden="true"></i> <span>Povratak</span></a>
											</div>
											<div class="text-center">
												<a style="float: right; margin-right: 8px;" class="prebacivanje_oznacenih btn material-btn material-btn-icon-success material-btn_success main-container__column material-btn-icon-responsive"><i style="color: white;" class="fa fa-tty idk_color_green aria-hidden="true"></i> <span>Prebaci kandidate odabranih agenata na skladište</span></a>
											</div>
				...
				<script>
							...
							<div class="row">
								<table id="arhivirani_tabela" class="display" cellspacing="0" width="100%">
										<thead>
											<tr>
												<th> <input type="checkbox" id="select_all" name="select_all"></th>
												<th class="text-center">Ime i prezime</th>
												<th class="text-center">Broj kandidata</th>
												<th class="text-center text-success">Prikaz kandidata agenta</th>
										...
										<tbody>
											<?php
											$query = $db->prepare("
																SELECT emp.employee_id, emp.employee_firstname, emp.employee_lastname, emp.employee_status, emp.employee_image, 
																emp.employee_nostrifikacija_diploma, count(kan.zaduzen_zaposlenik_nd_kandidata) as br_kand
																FROM idk_employees emp 
																JOIN idk_nd_kandidata kan 
																ON emp.employee_id = kan.zaduzen_zaposlenik_nd_kandidata where emp.employee_status = 0 
																AND emp.employee_nostrifikacija_diploma IN (0,1) GROUP BY (emp.employee_id)");

											$query->execute();

											while($row = $query->fetch()){
												$emp_name = $row['employee_firstname']." ".$row['employee_lastname'];
												$br_kand = $row['br_kand'];
												$employee_id = $row['employee_id'];
											...
											?>
												<tr>
													<td class="text-center"><input class="checkbox" type="checkbox" name="selectedrows[<?php echo $employee_id; ?>]" value="<?php echo $employee_id; ?>"></td>
													<td class="text-center"><a href="<?php getSiteURL(); ?>dipl_agenti?page=open_agent&id=<?php echo $employee_id; ?>"><?php echo $emp_name; ?></a></td>
													<td class="text-center"><?php echo $br_kand;?></td>
													<td class="text-center"><a class="btn material-btn material-btn-icon-success material-btn_success main-container__column material-btn-icon-responsive" target="_blank" href="<?php getSiteURL(); ?>nostrifikacija_diploma?page=menadzer_pregled_kandidata&id_m=<?php echo $employee_id; ?>"><i class="fa fa-arrow-right" aria-hidden="true"></i> Otvori</a></td>													
												</tr>
