...
$(document).ready(function() {	
		...
		$( ".edit_candidate_data" ).click(function() {
		
		var id = $(this).data('id'); 
		var type = $(this).data('type'); 
		var kandidat_id = $(this).data('kandidat'); 
		
		$('#loaderContent').html('');
		$('#loader').show();    
		
		$.ajax({
			url: 'ajax.php?page='+type+'',
			type: 'POST',
			data: {'id': id, 'kandidat_id': kandidat_id},
			dataType: 'html',
		})
		.done(function(data){
			setTimeout(function() { 
			//console.log(data); 
			$('#loaderContent').html(''); 
			$('#loaderContent').html(data);
			$('#loader').hide();
			}, 300);  

		})
		.fail(function(){
			$('#loaderContent').html('<i class="glyphicon glyphicon-info-sign"></i> Something went wrong, Please try again...');
			$('#loader').hide();
		});
...
$(document).ready(function() {
	if (document.getElementById("getCandidateGroup")) {
		
		var id = $('#getCandidateGroup').data('id');
		
		$('#getCandidateGroup').html('');
		$('#loader').show();    
		$.ajax({
			url: 'ajax.php?page=candidate_group',
			type: 'POST',
			data: {'id': id},
			dataType: 'html',
		})
		.done(function(data){
			...
			//console.log(data); 
			$('#getCandidateGroup').html(''); 
			$('#getCandidateGroup').html(data);
			$('#loader').hide();
			}, 1);  

		})
		.fail(function(){
			$('#getCandidateGroup').html('<i class="glyphicon glyphicon-info-sign"></i> Something went wrong, Please try again...');
			$('#loader').hide();
		});
	}
});
