...
case "getNumberOfNotifications":
	...
	$query = $db->prepare("
					...
					WHERE notification_employeeid = :notification_employeeid AND notification_status = :notification_status AND notification_datetime <= 'NOW()'");
...
case "getMessages_10":
	...
	$inbox_query = $db->prepare("
							SELECT message_id, message_subject, message_datetime, employee_firstname, employee_lastname, employee_image, mu_status
							FROM idk_messages
	...
	while($inbox_row = $inbox_query->fetch()){
		...
		if($inbox_row['employee_image'] == "none"){
		...
		echo '<li ' . $mess_status_style . '>
			<a href="' . getSiteURLr() . 'messages?page=open&id=' . $message_id . '">
				<div class="pull-left"><img src="' . getSiteURLr() . 'files/employees/' . $employee_image . '" class="img-circle" alt="User Image"></div>
...
if (isset($_POST['id']) && !empty($_POST['id'])) {
	...
	$query = $db->prepare("
							SELECT pt_name, pt_desc, pt_datetime, employee_firstname, employee_lastname
