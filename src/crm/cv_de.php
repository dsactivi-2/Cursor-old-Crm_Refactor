...
<body>
		...
		?>
        <form action="<?php getSiteURL(); ?>do.php?form=cv_profil_gen_de" name="frm-exp" id="frm-exp" method="POST">
			...
			<div class="row">
							...
							<ul class="fa-ul">
								...
								<?php
									...
									while($select_row = $select_query->fetch()) {
                                        ...
                                        $kki_naziv_de = $gt->translate($kki_naziv, 'de','hr');

                                        /*$query = $db->prepare("
                                            UPDATE idk_kandidat_kontakt_info
                                                SET	kki_naziv_de = :kki_naziv_de
                                                WHERE kki_id = :kki_id");

                                        $query->execute(array(
                                                ':kki_naziv_de' => $kki_naziv_de,
                                                ':kki_id' => $kki_id));*/
										
										
								?>
								<li><i class="fa-li fa <?php echo $ikona; ?>" aria-hidden="true"></i><p><?php echo $kki_naziv_de; ?>: <?php echo $kki_podatak; ?></p></li>
								<?php } ?>
				...
				<?php
					$select_query = $db->prepare("
										SELECT kri_id, kri_darum_od, kri_datum_do, kri_pozicija, kri_pozicija_de, kri_naziv, kri_grad, kri_opis, kri_aktuelno
				...
				<?php
					$select_query = $db->prepare("
										SELECT ke_id, ke_datumod, ke_datumdo, ke_naziv_kvalifikacije, ke_naziv_kvalifikacije_de, ke_naziv, ke_grad, ke_opis, ke_aktuelno
										...
										<tbody>
											<?php
												$select_query = $db->prepare("
																	...
																	WHERE kj_kandidatid = :kj_kandidatid AND kj_naziv = 'Njemački' ");
												...
												while($select_row = $select_query->fetch()) {
													...
													$query_ke->execute(array(
															...
															':kj_id' => $kj_id));*/
													//print_r($query_ke->errorInfo());
													
						
											?>
											<tr>

												<td class="text-center"><?php echo "Deutsch"; ?></td>
			...
			<div class="form-group">
				<div class="col-sm-offset-2 col-sm-10 text-right">
					<a href="<?php getSiteURL(); ?>do.php?form=cv_profil_gen_de&kand_id=<?php echo $kandidat_id; ?>" class="btn material-btn material-btn-icon-success material-btn_success main-container__column"><i class="fa fa-plus" aria-hidden="true"></i> <span>Spremi PDF</span></button>
					<a href="<?php getSiteUrl(); ?>cv_de?page=edit&id=<?php echo $kandidat_id; ?>" class="btn material-btn material-btn-icon-success material-btn_success main-container__column" style="margin-left: 20px;"><i class="fa fa-edit" aria-hidden="true"></i> <span>Uredi prijevod</span></button>
				</div>
			</div>		
		<?php
				break;	
		...
		?>
			<div class="row">
				<div class="col-xs-8">
					<h1><i class="fa fa-file-o idk_color_green" aria-hidden="true"></i> Uredi CV prijevod za <?php echo $kandidat_ime.' '.$kandidat_prezime; ?></h1>
				</div>
				<div class="col-xs-12">
					<hr />
				</div>
					...
					<div class="content_box">
						<div class="row">
							<div class="col-md-offset-1 col-md-8">
								<form id="idk_form" action="<?php getSiteURL(); ?>do.php?form=cv_de_edit" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
									...
									<hr/>
									<div class="form-group">
										<h4 class="col-sm-3 control-label text-right" style="color:#015625!important;"><b><i class="fa fa-angle-down" aria-hidden="true"></i> Radno iskustvo</b></h4>
										<div class="col-sm-9"></div>
									</div>
									<?php
									$select_query_iskustvo = $db->prepare("
										SELECT kri_id, kri_pozicija, kri_pozicija_de, kri_opis, kri_opis_de, kri_darum_od, kri_datum_do, kri_naziv_de, kri_grad_de, kri_naziv, kri_grad, kri_aktuelno
										FROM idk_kandidat_radno_iskustvo
										WHERE kri_kandidat_id = :kri_kandidat_id
									...
									while($select_row = $select_query_iskustvo->fetch()) {
										...
										if($select_row['kri_opis_de'] != NULL){
											$kri_opis = $select_row['kri_opis_de'];	
										}else{
											$kri_opis = $select_row['kri_opis'];	
										}										
										
										if($select_row['kri_pozicija_de'] != NULL){
											$kri_pozicija = $select_row['kri_pozicija_de'];	
										}else{
											$kri_pozicija = $select_row['kri_pozicija'];	
										}										
										
										if($select_row['kri_naziv_de'] != NULL){
											$kri_naziv = $select_row['kri_naziv_de'];	
										}else{
											$kri_naziv = $select_row['kri_naziv'];	
										}
										
										if($select_row['kri_grad_de'] != NULL){
									...
									?>
									<div class="form-group">
										<label for="kri_pozicija_opis" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Od - Do:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" disabled value="<?php echo $kri_darum_od_f; ?> - <?php echo $kri_datum_do_f; ?>">
												<span class="materail-input-block__line"></span>
											</div>											
										</div>
										<label for="kri_pozicija_opis" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Pozicija:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="kri_pozicija<?php echo $kri_id; ?>" id="kri_pozicija<?php echo $kri_id; ?>" value="<?php echo $kri_pozicija; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>
										
										<label for="kri_pozicija_opis" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Poslodavac:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="kri_naziv<?php echo $kri_id; ?>" id="kri_naziv<?php echo $kri_id; ?>" value="<?php echo $kri_naziv; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>

										<label for="kri_pozicija_opis" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Grad:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="kri_grad<?php echo $kri_id; ?>" id="kri_grad<?php echo $kri_id; ?>" value="<?php echo $kri_grad; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>										
										
										<label for="kri_pozicija_opis" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Pozicija (baza):</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<select class="form-control materail-input materail-input-custom" name="kp_id<?php echo $kri_id; ?>" id="kp_id<?php echo $kri_id; ?>" data-live-search="true" onchange="promjena_pozicije(this)" required>
													<option value="0">Bez pozicije</option>
										...
										<script>
										function promjena_pozicije(t){
											var idRI = t.id.slice(5);
											var pozicija = document.getElementById("kp_id"+idRI).value;
											console.log(idRI)
											var opisID = "kri_opis"+idRI;
											var opisID2 = <?php echo $kri_id; ?>;
											$.ajax({
												url: 'ajax.php?page=get_Position_Desc',
												type: 'POST',    
												data: {'pozicija':pozicija, 'kri_id':opisID2},
												dataType: 'html',
												success: function(data) {
													console.log(this);
													$("#opisID").text(data);
													var rezultat = data.slice(35);
													document.getElementById(opisID).innerHTML = rezultat;
												}
											});
										}
										</script>
										
										<label for="kri_pozicija_opis" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Opis:</label>
										<div class="col-sm-9">										
											<div class="materail-input-block materail-input-block_success">
												<textarea class="form-control materail-input materail-input-custom" type="text" rows=6 name="kri_opis<?php echo $kri_id; ?>" id="kri_opis<?php echo $kri_id; ?>" value="<?php echo $kri_opis; ?>"><?php echo $kri_opis; ?></textarea>
												<span class="materail-input-block__line"></span>
											</div>
										</div>
									</div>
									...
									<hr/>
									<div class="form-group">
										<h4 class="col-sm-3 control-label text-right" style="color:#015625!important;"><b><i class="fa fa-angle-down" aria-hidden="true"></i> Edukacija</b></h4>
										<div class="col-sm-9"></div>
									</div>									
									<?php
									$select_query_obrazovanje = $db->prepare("
										SELECT ke_id, ke_naziv_kvalifikacije, ke_naziv_kvalifikacije_de, ke_opis, ke_opis_de, ke_datumod, ke_datumdo, ke_naziv, ke_naziv_de, ke_grad, ke_grad_de, ke_aktuelno
										FROM idk_kandidat_edukacija
										WHERE ke_kandidat_id = :ke_kandidat_id
									...
									while($select_row = $select_query_obrazovanje->fetch()) {
										...
										if($select_row['ke_opis_de'] != NULL){
											$ke_opis = $select_row['ke_opis_de'];	
										}else{
											$ke_opis = $select_row['ke_opis'];	
										}											

										if($select_row['ke_naziv_kvalifikacije_de'] != NULL){
											$ke_naziv_kvalifikacije = $select_row['ke_naziv_kvalifikacije_de'];	
										}else{
											$ke_naziv_kvalifikacije = $select_row['ke_naziv_kvalifikacije'];	
										}										

										if($select_row['ke_naziv_de'] != NULL){
											$ke_naziv = $select_row['ke_naziv_de'];	
										}else{
											$ke_naziv = $select_row['ke_naziv'];	
										}
										
										if($select_row['ke_grad_de'] != NULL){
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" disabled value="<?php echo $ke_datumod_f; ?> - <?php echo $ke_datumdo_f; ?>">
												<span class="materail-input-block__line"></span>
											</div>											
										</div>								
										<label for="ke_edukacija" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Izučeno zanimanje:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="ke_naziv_kvalifikacije<?php echo $ke_id; ?>" id="ke_naziv_kvalifikacije<?php echo $ke_id; ?>" value="<?php echo $ke_naziv_kvalifikacije; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>
										
										<label for="ke_edukacija" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Škola:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="ke_naziv<?php echo $ke_id; ?>" id="ke_naziv<?php echo $ke_id; ?>" value="<?php echo $ke_naziv; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>		

										<label for="ke_edukacija" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Grad:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="ke_grad<?php echo $ke_id; ?>" id="ke_grad<?php echo $ke_id; ?>" value="<?php echo $ke_grad; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>											
										
										<label for="ke_edukacija" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Opis:</label>
										<div class="col-sm-9">										
											<div class="materail-input-block materail-input-block_success">
												<textarea class="form-control materail-input materail-input-custom" type="text" rows=6 name="ke_opis<?php echo $ke_id; ?>" id="ke_opis<?php echo $ke_id; ?>" value="<?php echo $ke_opis; ?>"><?php echo $ke_opis; ?></textarea>
												<span class="materail-input-block__line"></span>
											</div>
										</div>
									</div>
									...
									<!--
									<div class="form-group">
										<h4 class="col-sm-3 control-label text-right" style="color:#015625!important;"><b><i class="fa fa-angle-down" aria-hidden="true"></i> Vještine</b></h4>
										<div class="col-sm-9"></div>
									</div>	
									-->
									<?php /*
								...
								//		
									...
									<!--
									<div class="form-group">
										<label for="kv_vjestine" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Naziv:</label>
										<div class="col-sm-9">
											<div class="materail-input-block materail-input-block_success">
												<input class="form-control materail-input materail-input-custom" type="text" name="kv_naziv<?php echo $kv_id; ?>" id="kv_naziv<?php echo $kv_id; ?>" value="<?php echo $kv_naziv; ?>">
												<span class="materail-input-block__line"></span>
											</div>
										</div>
										<label for="kv_vjestine" class="col-sm-3 control-label" style="font-weight: 500!important;color: #6d6d6d;font-size: 13px;">Opis:</label>
										<div class="col-sm-9">										
											<div class="materail-input-block materail-input-block_success">
												<textarea class="form-control materail-input  materail-input-custom" type="text" rows=6 name="kv_opis<?php echo $kv_id; ?>" id="kv_opis<?php echo $kv_id; ?>" value="<?php echo $kv_opis; ?>"><?php echo $kv_opis; ?></textarea>
												<span class="materail-input-block__line"></span>
											</div>
										</div>
									</div>
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="col-xs-4 text_right">
						<form id="idk_formXX" action="<?php getSiteURL(); ?>do.php?form=cv_gen_de" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
							<input type="hidden" name="kand_id" value="<?php echo $kandidat_id; ?>" />
