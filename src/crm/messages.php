	...
	<div id="content">
										...
										<tbody>
											<?php
												$inbox_query = $db->prepare("
																		SELECT message_id, message_subject, message_datetime, employee_firstname, employee_lastname, mu_status
																		FROM idk_messages
																		INNER JOIN idk_messages_users ON idk_messages.message_id = idk_messages_users.mu_messageid
																		INNER JOIN idk_employees ON idk_messages.message_sentid = idk_employees.employee_id
												...
												while($inbox_row = $inbox_query->fetch()){
													...
													$employee_fullname = $inbox_row['employee_firstname'] . ' ' . $inbox_row['employee_lastname'];
											...
											<tr>
												...
												<td onclick="document.location = 'messages?page=open&id=<?php echo $message_id; ?>';" style="<?php echo $mess_status_style; ?> cursor: pointer;"><?php echo $employee_fullname; ?></td>
										...
										<tbody>
											<?php
												$inbox_query = $db->prepare("
																		...
																		FROM idk_messages
																		INNER JOIN idk_messages_users ON idk_messages.message_id = idk_messages_users.mu_messageid
																		INNER JOIN idk_employees ON idk_messages_users.mu_employeeid = idk_employees.employee_id
											  ...
											  <select class="js-basic-multiple form-control" multiple="multiple" id="mu_employeeid" name="mu_employeeid[]" required>
					  							<?php
					  								...
					  								while($users = $users_query->fetch()) {
					  									...
					  									echo "<option value='" . $users['employee_id'] . "'>" . $users['employee_firstname'] . " " . $users['employee_lastname'] . "</option>";
					  								}
					  							?>
					  						</select>
					  						<script type="text/javascript">
		...
		<?php
				...
				if($number_of_messages > 0 OR $number_of_sent_messages > 0){
					...
					//Get message data
					$query = $db->prepare("
									SELECT message_id, message_subject, message_text, message_datetime, employee_firstname, employee_lastname, employee_image, mu_status
									FROM idk_messages
									INNER JOIN idk_messages_users ON idk_messages.message_id = idk_messages_users.mu_messageid
									INNER JOIN idk_employees ON idk_messages.message_sentid = idk_employees.employee_id
					...
					$row = $query->fetch();
						...
						$employee_fullname = $row['employee_firstname'] . ' ' . $row['employee_lastname'];
					 ...
					 <div class="row">
						 <div class="col-md-offset-1 col-md-10">
							 <div class="row">
							 	<div class="col-sm-8">
									<ul class="list-inline">
										<li><a class="fancybox" rel="group" href="<?php getSiteURL(); ?>files/<?php echo getSubdomainr(); ?>_files/employees/<?php echo $employee_image; ?>"><img class="idk_profile_img" src="<?php getSiteURL(); ?>files/<?php echo getSubdomainr(); ?>_files/employees/<?php echo $employee_image; ?>"></a></li>
												...
												<select class="js-basic-multiple form-control" multiple="multiple" id="mu_employeeid" name="mu_employeeid[]" required>
												  <?php
													  ...
													  while($users = $users_query->fetch()) {
														  ...
														  echo "<option value='" . $users['employee_id'] . "' " . $user_select . ">" . $users['employee_firstname'] . " " . $users['employee_lastname'] . "</option>";
													  }
												  ?>
											  </select>
											  <script type="text/javascript">
