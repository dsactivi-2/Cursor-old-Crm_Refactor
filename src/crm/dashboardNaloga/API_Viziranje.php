...
switch ($page) {
    case 'allCompanies':
        $sql = "
                SELECT
                    ...
                    SUM(IF(kandidat_status_prijave = 21 OR kandidat_status_prijave = 24,    qty, 0)) AS dopuna_odbijenica,
                ...
                FROM
                    (
                        ...
                        WHERE kandidat_status_prijave IS NOT NULL
                        GROUP BY company_id , kandidat_status_prijave
                    ) AS res
                        ...
                        FROM
                            (
                                ...
                                FROM
                                    `idk_nalozi`
                                WHERE
                                    `idk_nalozi`.`kompanija_id` = :id AND `idk_nalozi`.`pristup_poslodavcima` = 1 and  nalog_status!= 12 
                            ) AS nalozi
                        INNER JOIN `idk_kandidati` ON `idk_kandidati`.`kandidat_nalog_id` = `nalozi`.`nalog_id`
                        WHERE `idk_kandidati`.`kandidat_status_prijave` IS NOT NULL
                        GROUP BY nalog_id, kandidat_status_prijave
                    ) AS res
                GROUP BY nalog_id
        ";
    ...
    case "nalogSvi":
        ...
        if (isset($_REQUEST["statusPrijave"])) {
            ...
            if($statusPrijave == 21){
                $statusPrijaveUslov =$statusPrijaveUslov." AND `kandidat_status_prijave` in (21,24) ";
        ...
        $sql = "
            SELECT
                ...
                nalog_naziv,
                status_naziv,
                MIN(`idk_log_statusi_prijave`.`lsp_datetime`) as na_statusu_od
            FROM
                `idk_nalozi`
            ...
            WHERE
                `idk_kandidati`.`kandidat_status_prijave` IN (5,10,12,15,18,21,24,27)
            AND
