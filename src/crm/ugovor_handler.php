...
switch ($action){
	...
	case "poziv":
		...
		if(is_null($kandidat_id)){
			...
			generisiUgovor($kandidat_id, $jezik);
		}
	...
	case "prihvacen":
		...
		if($ug_status == 1 || $ug_status == 4){
			...
			$ugovor_link = getSiteUrlr()."ugovor/".$token."/".$ug_jezik;
			if($ug_jezik == "sr"){
			...
			sendMailUgovorPredracun($kandidat_id, $poruka_text_mail, $ugovor_link, $subject_ugovor);
			generisiUgovor($kandidat_id, $jezik);
		}
		else{
	...
	case "generisi_ugovor":
		...
		generisiUgovor($row_get_kandidat['ug_kandidat_id'], "de");
	break;
	...
	case "kopija_de":
		...
		$token = $_GET['token'];
		$jezik = $_GET['jezik'];
		$ug_file_de = $_GET['fileName'];
			
			
		$query_update_ugovor_kandidata = $db -> prepare("
			UPDATE idk_nd_ugovori 
			SET ug_file_de = :ug_file_de
			WHERE ug_token = :token
		");

		$query_update_ugovor_kandidata -> execute(array(
			':ug_file_de' => $ug_file_de,
			':token' => $token
		));
		
		$query_get_kandidat_id = $db -> prepare("
			SELECT ug_kandidat_id, ug_file 
			FROM idk_nd_ugovori 
			WHERE ug_token = :token
		");
		...
		$ug_file = $row_get_kandidat_id['ug_file'];

		generisiUgovor($ug_kandidat_id, $jezik);
	break;
										...
										<div class="row">
											<div class="col-md-offset-2 col-md-8">
												<div class="panel panel-default">
													...
													?>
													<div class="panel-body">
														<form action="<?php  echo getSiteUrlr().'ugovor_handler.php?action=spremi_razlog';?>" id="form_spremi_razlog" method="post" enctype="multipart/form-data"  class="form-horizontal" role="form">
	...
	case "spremi_razlog":
		...
		$url = getSiteUrlr();																														
		$url = $url.'ugovor_handler.php?action=odbijen_razlogom&token='.$token.'&jezik='.$jezik;	
		header("Location:".$url);
	...
	case 'generisi_predracun_de':
		$token 		= $_GET['token'];
		$jezik 		= $_GET['jezik'];
		$ug_file 	= $_GET['fileName'];
		
		$url = getSiteUrlr();
		$url = $url.'ugovor_handler.php?action=typage&token='.$token.'&jezik='.$jezik.'&fileName='.$ug_file;
